...
export const koCases: readonly CaseStudy[] = [
  {
    ...
    summary:
      "미국 배관·전기 트레이드 사업자를 위한 보이스 AI 백오피스. 일정·고객·인보이스를 음성 한 문장으로 처리하는 제품을 웹·앱·백엔드·클라우드까지 풀스택으로 설계하고 구현했습니다.",
    impact: [
      "13개 proto 기반 247개 gRPC 엔드포인트 설계",
      "Next.js 관리자 + Flutter 앱 모노레포 동시 개발",
    ...
    detail: {
      label: "상세 스펙",
      overview:
        "현장 작업자가 손을 쓰지 않고 음성으로 일정·고객·인보이스를 관리하도록, 계약형 백엔드와 멀티플랫폼 앱, 클라우드 배포를 하나의 제품 흐름으로 묶은 케이스입니다.",
      specs: [
  ...
  },
  {
    ...
    summary:
      "VIV 스마트링 제품을 중심으로 iOS 앱, NestJS API, 로컬 Swift 패키지, TestFlight 배포 준비까지 제품 단위로 정리한 케이스입니다.",
    impact: ["스마트링 앱 구조 설계", "NestJS API와 DB 흐름 정리", "iOS 빌드·패키지·배포 준비"],
    ...
    detail: {
      label: "상세 스펙",
      overview:
        "웨어러블 링 앱과 서버 경계를 제품 단위로 정리해 앱·API·DB·배포 준비가 한 흐름에서 보이게 만든 케이스입니다.",
      specs: [
      ...
      delivery: ["스마트링 앱 구조 설계", "API·DB 책임 경계 정리", "iOS 빌드와 배포 준비"],
    },
  },
  {
    id: "smartcharge",
    ...
    summary:
      "스마트기기 충전 호환성 테스트를 관리하는 운영 콘솔과 API 흐름을 설계한 프로젝트입니다. 운영자가 데이터를 빠르게 비교하고 의사결정하도록 정보구조를 잡았습니다.",
    impact: ["운영 콘솔 UX·정보구조 설계", "OpenAPI 기반 API 문서화", "PostgreSQL 데이터 모델링"],
    ...
    detail: {
      label: "상세 스펙",
      overview:
        "충전 호환성 데이터를 운영자가 빠르게 비교하고, 테스트 큐와 API 계약을 함께 확인하는 관리 콘솔입니다.",
      specs: [
  ...
  },
  {
    ...
    summary:
      "1,000명 이상의 크리에이터와 8,000명 이상의 후원자가 사용한 소셜 크라우드펀딩 앱. 레거시 구조를 클린 아키텍처로 재설계하고 결제·실시간·자동 배포까지 운영 가능한 상태로 만들었습니다.",
    impact: [
      "GetX→Riverpod 클린 아키텍처 마이그레이션(35개 화면)",
      "Toss 결제·실시간 채팅·FCM 푸시 구현",
      "Fastlane·GitHub Actions로 스토어 자동 배포",
    ],
    ...
    detail: {
      label: "상세 스펙",
      overview:
        "레거시 앱을 데이터·도메인·프레젠테이션 3계층으로 재설계해 기능 개발 속도를 약 40% 높이고, 결제·실시간·배포 파이프라인을 정리해 안정적으로 운영되게 만든 케이스입니다.",
      specs: [
